...
**Current state:** ~3430 lines of C, fully working, committed as `a7a0e76` and pushed.
...
### 8.1 cockpit-gst-manager Changes

| File | Change |
|------|--------|
| `backend/discovery.py` | Add detection for `/dev/video_cap` + streambox-src plugin |
| `backend/auto_instance.py` | Switch from `v4l2src device=/dev/video71` to `streambox-src` |
| `backend/events.py` | Option to get events from V4L2 instead of tvserver polling |
| `backend/tvservice.py` | Retain as-is (tvserver still controls display path) |
...
**Commit history:**
- `8d0a6bf` -- initial libvfmcap add
- `97e9276` -- BT.2020->BT.709 CSC for NV12 (superseded)
- `dddffa0` -- 3-pipeline split with HDR (superseded)
- `0c432c1` -- remove HDR/CSC code, simplify to raw-only
...
**Commit history:**
- `fb8c48e` -- gst-plugin-vfmcap: add streamboxsrc dual-path GStreamer plugin
- `ec9f1ac` -- streamboxsrc: fix allocation, DMA-buf output for Path B, fd leak
- `f60842a` -- streamboxsrc: Path B zero-copy via V4L2_MEMORY_DMABUF with CMA DMA-bufs
- `765b310` -- streamboxsrc: add Path B 10-bit P010 output via Vulkan AMLY->P010 conversion
- `c9ca5a2` -- streamboxsrc: double-buffered async GPU for Path B 10-bit (39fps -> 60fps)
- `461125c` -- amlvenc: fix VBR/CBR INT_BSBUF_FULL by propagating bitstream buffer size to hardware
- `1d592da` -- streamboxsrc: fix vdin1 AMLY field mapping -- swap Y<->UV in decode_pair
- `1f3007b` -- fix P010 encoding: use CMA DMA-bufs and Wave521 MSB byte-swap format
- `c0e0d61` -- streamboxsrc: fix Path A fd recycling with rotating CMA output buffer pool
- `b1e2f35` -- amlvenc (submodule): propagate encoder-buffer-size to hardware bitstream DMA buffer
...
**Commit history:**
- `f37d1b2` -- streamboxsrc: add event-driven signal monitor for Path B via vfm_cap
- `a7a0e76` -- streamboxsrc: clean exit on signal change -- post message + EOS instead of in-place reconfigure
...
- [ ] cockpit-gst-manager updates (discovery, auto-instance, events)
...
|------|--------|-------------|
| `aml-comp/hardware/aml-5.15/amlogic/vfm_cap/vfm_cap.c` | Done | Kernel module (~2020 lines) |
| `aml-comp/hardware/aml-5.15/amlogic/vfm_cap/vfm_cap.h` | Done | Module header |
| `aml-comp/hardware/aml-5.15/amlogic/vfm_cap/Makefile` | Done | Kernel build |
| `aml-comp/hardware/aml-5.15/amlogic/vfm_cap/vfm-cap-setup.sh` | Done | Setup script |
| `aml-comp/hardware/aml-5.15/amlogic/vfm_cap/test_dmabuf.c` | Done | DMA-buf test |
| `meta-meson/recipes-kernel/vfm-cap/vfm-cap_1.0.bb` | Done | Yocto recipe |
| `meta-meson/recipes-kernel/vfm-cap/files/99-vfm-cap.rules` | Done | udev rule |
| `meta-meson/conf/machine/include/amlogic_externalsrc.inc` | Done | EXTERNALSRC |
| `aml_tvserver_streambox/libtv/CTvin.cpp` | Done | VFM chain insert |
| `aml-comp/multimedia/libvfmcap/include/vfmcap.h` | Done | Public API |
| `aml-comp/multimedia/libvfmcap/src/vfmcap.c` | Done | V4L2 capture core |
| `aml-comp/multimedia/libvfmcap/src/vfmcap_vulkan.c` | Done | Vulkan converter |
| `aml-comp/multimedia/libvfmcap/src/vfmcap_vulkan.h` | Done | Internal header |
| `aml-comp/multimedia/libvfmcap/shaders/amly_to_p010.comp` | Done | P010 shader |
| `aml-comp/multimedia/libvfmcap/shaders/amly_to_nv12.comp` | Done | NV12 shader |
| `aml-comp/multimedia/libvfmcap/demo/vfmcap-demo.c` | Done | Demo program |
| `aml-comp/multimedia/libvfmcap/Makefile` | Done | Build system |
| `meta-meson/recipes-multimedia/libvfmcap/libvfmcap_1.0.bb` | Done | Yocto recipe |

### Phase 4: streambox-src Plugin (COMPLETE)

| File | Status | Description |
|------|--------|-------------|
| `aml-comp/multimedia/gst-plugin-vfmcap/gst_streambox_src.c` | Done | Source element (~3430 lines) |
| `aml-comp/multimedia/gst-plugin-vfmcap/gst_streambox_src.h` | Done | Plugin header (~200 lines) |
| `aml-comp/multimedia/gst-plugin-vfmcap/Makefile` | Done | Plugin build |
| `aml-comp/multimedia/gst-plugin-vfmcap/vdin1_amly_to_p010.comp` | Done | Vulkan compute shader: vdin1 AMLY -> Wave521 MSB P010 |
| `aml-comp/multimedia/gst-plugin-vfmcap/vdin1_amly_to_p010.spv` | Done | Compiled SPIR-V binary |
| `aml-comp/multimedia/gst-plugin-vfmcap/vdin1_amly_to_p010_spv.h` | Done | C header with embedded SPIR-V |
| `aml-comp/multimedia/gst-plugin-venc/multienc-wave521/gstamlvenc_multienc.c` | Done | amlvenc: VBR/CBR bitstream buffer fix |
...
| File | Status | Description |
|------|--------|-------------|
| `cockpit-gst-manager/backend/discovery.py` | TODO | Detect new devices |
| `cockpit-gst-manager/backend/auto_instance.py` | TODO | Use streamboxsrc |
| `cockpit-gst-manager/backend/events.py` | TODO | V4L2 event source |
